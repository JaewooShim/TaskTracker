...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const api: AppContextType["api"] = {
    fetchTaskLists: async () => {
      const response = await apiClient.get<TaskList[]>(
        "/api/task-lists",
        jsonHeaders
      );
    ...
    updateTaskList: async (id, taskList) => {
      const response = await apiClient.put<TaskList>(
        `/api/task-lists/${id}`,
        taskList,
        jsonHeaders
      );
    ...
    deleteTaskList: async (id) => {
      await apiClient.delete(`/api/task-lists/${id}`, jsonHeaders);
    ...
    checkAuth: async () => {
      try {
        await authApi.get("/api/auth", { withCredentials: true });
        dispatch({ type: LOGIN });
        return true;
      } catch {
